...
## Integration With Podcast Feature

Episode به‌صورت مفهومی با Podcast در ارتباط است اما نباید ownership آن را نقض کند.

قواعد:

- Podcast feature نباید داخل episode implementation dependency ایجاد کند.
- Episode نباید UI primitive اختصاصی podcast بسازد.
- ارتباط بین آن‌ها از طریق shared contracts یا data layer انجام شود.
- صفحه podcast می‌تواند component episode را مصرف کند، اما مالک آن نباشد.
...
- مرز episode/shared ثبت شود.
- route logic از feature logic تفکیک شود.
- episode components مالکیت مشخص داشته باشند.
- dependency با podcast کنترل شود.
- player boundary مستقل باقی بماند.
- هر تغییر آینده با build/lint validation انجام شود.
